...
model2 <- lm(EnergyPDry_1_mJ_g~Collection_RiverMile_m + Group_Assignment, data = DataSet)
model3 <- lm(EnergyPDry_1_mJ_g~Collection_RiverMile_m + Sex, data = DataSet)
model4 <- lm(EnergyPDry_1_mJ_g~Collection_RiverMile_m + Group_Assignment + Sex, data = DataSet)
...
aic_df$ModelName <- c("EnergyPDry_1_mJ_g~Collection_RiverMile_m + Group_Assignment",
                      "EnergyPDry_1_mJ_g~Collection_RiverMile_m + Group_Assignment + Sex",
...
ggplot(GSI_estimates, aes(x = Avg_Work, y = Estimated_EnergyDensity, label = GSI)) +
  ...
  geom_text(nudge_y = 0.00008, size = 3) +
  labs(title = "Model-Averaged Estimated Energy Density vs. Migration Work",
       x = "Migration Work (km²)", y = "Estimated Energy Density (mJ/g)") +
  theme_minimal() + 
   theme(

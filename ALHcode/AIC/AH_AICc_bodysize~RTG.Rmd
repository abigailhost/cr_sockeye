...
```{r}
models <- list(
  lm(bodysize_pc~1, data=DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + RunTimingGroup, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + Sex, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + Year, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + RunTimingGroup + Sex, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + RunTimingGroup + Year, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + Sex + Year, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + RunTimingGroup + Sex + RunTimingGroup:Sex, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + Sex + Year + Sex:Year, data = DataSet),
  lm(bodysize_pc~Collection_RiverMile_m + RunTimingGroup + Sex + Year, data = DataSet)
)
...
g2 <- ggplot(estimates_body, aes(x = Avg_Coef, y = X)) + 
  geom_point(size = 3) +  # For the coefficient estimate as dots
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) + # Horizontal CI bars
  labs(x = "Parameter Estimate", y = "Parameter") +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", size = 1) +
  ...
  theme(
    ...
    plot.title = element_text(color = "black"),                            # Title in black (if you add a title)
    plot.subtitle = element_text(color = "black")                         # Subtitle in black (if you add a subtitle)
  )
...
# Create the plot
ggplot(estimates_body_filtered, aes(x = Avg_Coef, y = X)) + 
  geom_point(size = 3) +  # For the coefficient estimate as dots
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) + # Horizontal CI bars
  labs(x = "Parameter Estimate", y = "Parameter") +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", size = 1) +
...
ggsave("AMSS_2025/AMSS Body Size Parameter Estimates.png", plot = last_plot(), height = 6, width = 12, units = "in")

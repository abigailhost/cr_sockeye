#
# AH Host
# AICc code from Kristen, bodysize~RunTimingGroup test
# November 2024
#
#-----
...
lowerriver_bodycomp_all<-read.csv("ALHcode/AIC/lowerriver_bodycomp_all.csv")
...
#-----
# Testing for normality of Fish_Leng_1, but this is subject to change dependent on the metric we use for body size 

list.files()
lowerriver_bodycomp_all<-read.csv("ALHcode/AIC/lowerriver_bodycomp_all.csv")
lowerriver_bodycomp_all
nrow(lowerriver_bodycomp_all)

jpeg(file="ALHcode/AIC/Norm_plot_lowerriver_Fish_Leng_1.jpg")
qqnorm(lowerriver_bodycomp_all$Fish_Leng_1)
qqline(lowerriver_bodycomp_all$Fish_Leng_1)
dev.off()

jpeg(file="ALHcode/AIC/Hist_plot_lowerriver_Fish_Leng_1.jpg")
hist(lowerriver_bodycomp_all$Fish_Leng_1)
dev.off()

shapiro.test(lowerriver_bodycomp_all$Fish_Leng_1) #not normal!

#-----
# Open the file just created, and be sure to discard column, 1 and change all continuous variables to a num.
...
CoefName<- c("Intercept", "Intercept.SE", "Collection_Location", "Collection_Location.SE", "RunTimingGroup", "RunTimingGroup.SE", "Sex", "Sex.SE", "Year","Year.SE", "RunTimingGroup:Sex", "RunTimingGroup:Sex.SE", "RunTimingGroup:Year", "RunTimingGroup:Year.SE", "Sex:Year", "Sex:Year.SE")
...
AICModelMatrix<- as.data.frame(matrix(NA, nrow=length(ModelSet), ncol=length(CoefName)+11, dimnames=list(c(1:length(ModelSet)), c("Model", CoefName, "N.Obs", "k", "EDF", "RMSE", "SSE", "logLik", "-2logLik", "mul.r.squared", "AIC", "AIC.c"))))
...
for(i in 1:length(ModelSet)){
  ...
  AICModelMatrix[i,"Model"]<- ModelSet[i]
  AICModelMatrix[i,"Intercept"]<- coef(m)["(Intercept)"]
  AICModelMatrix[i,"Intercept.SE"]<- summary(m)$coef["(Intercept)",2]
  AICModelMatrix[i,"Collection_Location"]<- coef(m)["Collection_Location"]
  AICModelMatrix[i,"Collection_Location.SE"]<- summary(m)$coef[,2]["Collection_Location"]
  AICModelMatrix[i,"RunTimingGroup"]<- coef(m)["RunTimingGroup"]
  AICModelMatrix[i,"RunTimingGroup.SE"]<- summary(m)$coef[,2]["RunTimingGroup"]
  AICModelMatrix[i,"Sex"]<- coef(m)["Sex"]
  AICModelMatrix[i,"Sex.SE"]<- summary(m)$coef[,2]["Sex"]
  AICModelMatrix[i,"Year"]<- coef(m)["Year"]
  AICModelMatrix[i,"Year.SE"]<- summary(m)$coef[,2]["Year"]
  AICModelMatrix[i,"RunTimingGroup:Sex"]<- coef(m)["RunTimingGroup:Sex"]
  AICModelMatrix[i,"RunTimingGroup:Sex.SE"]<- summary(m)$coef[,2]["RunTimingGroup:Sex"]
  AICModelMatrix[i,"RunTimingGroup:Year"]<- coef(m)["RunTimingGroup:Year"]
  AICModelMatrix[i,"RunTimingGroup:Year.SE"]<- summary(m)$coef[,2]["RunTimingGroup:Year"]
  AICModelMatrix[i,"Sex:Year"]<- coef(m)["Sex:Year"]
  AICModelMatrix[i,"Sex:Year.SE"]<- summary(m)$coef[,2]["Sex:Year"]

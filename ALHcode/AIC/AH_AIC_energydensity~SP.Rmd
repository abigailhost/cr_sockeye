...
ggsave("energydensity_spawningpopulation/CopperRiverWS_EnergyYearSP_prespawn.jpeg", plot=g2, height=16, width=26, units="in")
...
### Pre vs Post Spawn energy differences in sockeye for 2021 only
# Ensure 'Year' is a factor variable (if not already)
DataSet_prepostspawn$Year <- as.factor(DataSet_prepostspawn$Year)

# Create the boxplot or violin plot with faceting by year
gspawn <- ggplot(DataSet_prepostspawn, aes(x = SpawnStatus, y = EnergyPDry_1, fill = factor(SpawnStatus))) +
  geom_boxplot(alpha = 0.6, size = 1.5, outlier.size = 5) + # Color for pre-spawn and post-spawn
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
ggsave("energydensity_spawningpopulation/CopperRiverWS_EnergyPrePostSpawn.jpeg", plot=gspawn, height=16, width=20, units="in")

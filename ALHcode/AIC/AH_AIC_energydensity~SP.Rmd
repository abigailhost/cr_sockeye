...
g1 <- ggplot(DataSet_prespawn, aes(x = Collection_Location, y = EnergyPDry_1)) +
  ...
  labs(title = "Energy Content of Pre-Spawn Fish at different Spawning Population",
...
g2 <- ggplot(DataSet_prespawn, aes(x = Collection_Location, y = EnergyPDry_1)) +
  ...
  facet_wrap(~ Year) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Energy Content of Pre-Spawn fish across Spawning Population, separated by Year",
       x = "Collection Location",
       y = "Energy Content (mJ/kg)") + 
   guides(fill = "none")
...
g3 <- ggplot(data_summary, aes(x = Year, y = `Energy Content`, group = Collection_Location, color = Collection_Location)) +
  geom_line() +
  geom_point() +
  labs(title = "Trends of Mean Energy Content of Pre-Spawn Fish across Year and Collection Location",
       x = "Year",
       y = "Mean Energy Content (mJ/kg)") +
  theme_minimal()

ggsave("energydensity_spawningpopulation/CopperRiverWS_MeanEnergyYearSP_prespawn.png", plot=g3, height=16, width=20, units="in")

...
bc_pca2 <- princomp(lowerriver_GSI_all[,c(85,86,87)], cor=T, scores=T, covmat = NULL) #should work now without any NAs, identical scores to that of the one above.  units do not matter!
...
### Using summed weighted parameter estimates for body size by GSI ~ work (successful size for migration, as continuous variable)
```{r}
...
GSI_effects$Estimated_BodySize <- intercept + GSI_effects$Avg_Coef
...
bodysize_work_model <- lm(Estimated_BodySize ~ Avg_Work, data = GSI_estimates)
summary(bodysize_work_model)

# Step 9: Visualization
library(ggplot2)
ggplot(GSI_estimates, aes(x = Avg_Work, y = Estimated_BodySize, label = GSI)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  geom_text(nudge_y = 0.05, size = 3) +
  labs(title = "Model-Averaged Estimated Body Size vs. Migration Work",
       x = "Migration Work (km²)", y = "Estimated Body Size (PC Score)") +
  theme_minimal() + 
   theme(
...
bodysize_work_model_noChitina <- lm(Estimated_BodySize ~ Avg_Work, data = GSI_estimates_noChitina)
summary(bodysize_work_model_noChitina)

# Step 9: Visualization
library(ggplot2)
ggplot(GSI_estimates_noChitina, aes(x = Avg_Work, y = Estimated_BodySize, label = GSI)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  geom_text(nudge_y = 0.05, size = 3) +
  labs(title = "Model-Averaged Estimated Body Size vs. Migration Work",
...
### Julian Day figure for body size across date, with GSI
```{r}
library(lubridate)
...
ggplot(DataSet, aes(x = JulianDay, 
  ...
  labs(x = "Collection Date",
       y = "Estimated Body Size (PC Score)",
...
### Characterizing Group Assignment (GSI) for run-timing groups
```{r}
DataSet$RunTimingGroup <- factor(
  DataSet$RunTimingGroup,
  levels = c("Early", "Middle", "Late")
)
# Table of counts
table(DataSet$Group_Assignment, DataSet$RunTimingGroup)



DataSet$Group_Assignment <- factor(
  DataSet$Group_Assignment,
  levels = c("LowerGroups",
             "Bremner",
             "Chitina",
             "KlutinaLake",
...
my_colors <- c(
  "#774762FF", "#D9AF6BFF", "#AF6458FF", "#736F4CFF", "#526A83FF", "#625377FF", "#68855CFF", "#9C9C5EFF", "#A06177FF", "#467378FF"
)
#Visual aid of counts
ggplot(DataSet, aes(x = RunTimingGroup, fill = Group_Assignment)) +
  geom_bar(position = "stack") +
  theme_minimal() +
  labs(
    title = "GSI Distribution Within Run Timing Groups",
    x = "Run Timing Group",
    y = "Count",
    fill = "GSI Group"
  ) + 
...
ggplot(DataSet, aes(x = RunTimingGroup, fill = Group_Assignment)) +
  ...
  theme_minimal() +
  labs(
    title = "Proportion of GSI Groups by Run Timing",
    x = "Run Timing Group",
    y = "Proportion",
    fill = "GSI Group"
  ) + 
...
GSI_summary_by_group <- DataSet %>%
  group_by(Group_Assignment, RunTimingGroup) %>%
...
# View it
print(GSI_summary_by_run)
library(knitr)
GSI_summary_by_group %>%
  kable(
    digits = 3,
    format = "html",
    caption = "GSI Summary Grouped by GSI (Group_Assignment)"
  ) %>%
  kable_styling(

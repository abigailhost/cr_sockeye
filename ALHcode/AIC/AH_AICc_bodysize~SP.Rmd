...
### Estimate Length_2 Values for NA rows
```{r}
#Determine best metric for estimating NAs for Leng_2
summary(lm(Fish_Leng_2 ~ Fish_Leng_1, data = upperriver_bodycomp_all)) # R =  0.05932 
summary(lm(Fish_Leng_2 ~ Fish_Grth, data = upperriver_bodycomp_all)) #R = 0.1022 
summary(lm(Fish_Leng_2 ~ Fish_Wt, data = upperriver_bodycomp_all)) #R =  0.09534
summary(lm(Fish_Leng_2 ~ Fish_Ht, data = upperriver_bodycomp_all)) # Adjusted R-squared =  0.113 
...
# pca_leng2 <- princomp(allF_upperriver_bodycomp_all[,c(12,16,19)], cor=T, scores=T, covmat = NULL)
# summary(pca_leng2, loadings=T, cutoff=0.001) #summary of PC analysis

#### Create a new dataframe with PCA scores for the female F fish subset
# allF_upperriver_bodycomp_all <- cbind(allF_upperriver_bodycomp_all, pca_leng2$scores[, 1])  # First principal component

### Create the linear model for predicting Fish_Grth based on the first principal component
# leng2_F_lm <- lm(Fish_Leng_2 ~ pca_leng2$scores[, 1], data = allF_upperriver_bodycomp_all)
# summary(leng2_F_lm)


# predicted_values_Leng2F <- predict(leng2_F_lm, newdata = allF_upperriver_bodycomp_all)

# allF_upperriver_bodycomp_all$Predicted_Fish_Ht <- predicted_values_HtF # Add these predictions back to your dataset


### ALL different regressions with leng_2 as response produce super low r^2 values == As of now, leaving leng_2 out of the body size metric for spawning population 
...
### Making body size metric via PC score of Leng_1, Fish_Ht, and Fish_Grth (potential to add Fish_Wdth)
```{r}
str(upperriver_bodycomp_all)

## Comparing 2 versions of PC scores via linear regression for body size analysis ### 
bc_pca1 <- princomp(upperriver_bodycomp_all[,c(12,16,19)], cor=T, scores=T, covmat = NULL) #should work now without any NAs
summary(lm(upperriver_bodycomp_all$Fish_Leng_1 ~ bc_pca1$scores[,1])) #R = 0.6382 

bc_pca2 <- princomp(upperriver_bodycomp_all[,c(12,16,18,19)], cor=T, scores=T, covmat = NULL) #should work now without any NAs
summary(lm(upperriver_bodycomp_all$Fish_Leng_1 ~ bc_pca2$scores[,1])) # R = 0.6475


summary(bc_pca2, loadings=T, cutoff=0.0001) #summary of PC analysis
bc_pca2$scores
screeplot(bc_pca2, type=c('lines'))
bodysize_pc <- bc_pca2$scores[,1]
...
upperriver_bodycomp_pc <- upperriver_bodycomp_pc[,-(81:84)]
str(upperriver_bodycomp_pc)
...
model5 <- lm(bodysize_pc~SpawnStatus + Collection_Location + Sex, data = DataSet)
model6 <- lm(bodysize_pc~SpawnStatus + Collection_Location + Year, data = DataSet)
model7 <- lm(bodysize_pc~SpawnStatus + Sex + Year, data = DataSet)
model8 <- lm(bodysize_pc~SpawnStatus + Collection_Location + Sex + Collection_Location:Sex, data = DataSet)
model9 <- lm(bodysize_pc~SpawnStatus + Sex + Year + Sex:Year, data = DataSet)
model10 <- lm(bodysize_pc~SpawnStatus + Collection_Location + Sex + Year, data = DataSet)
```
...
aic_df$ModelName <- c("bodysize_pc~SpawnStatus + Sex + Year",
                      "bodysize_pc~SpawnStatus + Sex + Year + Sex:Year",
                      "bodysize_pc~SpawnStatus + Sex",
                      "bodysize_pc~SpawnStatus + Collection_Location + Sex + Year",
                      "bodysize_pc~SpawnStatus + Collection_Location + Sex",
                      "bodysize_pc~SpawnStatus + Collection_Location + Sex + Collection_Location:Sex",
...
fineness_plot <- ggplot(paramestimates_bodysize_filtered, aes(x = Avg_Coef, y = X)) + 
  geom_point(size = 3) +  # For the coefficient estimate as dots
  geom_errorbarh(aes(xmin = CI_Lower, xmax = CI_Upper), height = 0.2) + # Horizontal CI bars
  labs(x = "Parameter Estimate", y = "Parameter") +
  geom_vline(xintercept = 0, color = "red", linetype = "dashed", size = 1) +
